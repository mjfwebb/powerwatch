...
| Windows WMI `BatteryStatus.DischargeRate` | The whole laptop (same coverage as `power_now`) | Used on Windows 11 (MSYS2/Git Bash, Cygwin, WSL). Queried via PowerShell `Get-CimInstance` once per tick. Reports instantaneous discharge in mW, which powerwatch multiplies to µW for the same integration path as Linux. |
